    ...
    auiDialog.prototype = {
        ...
        create: function(params,callback) {
            ...
            if(params.input){
                ...
                var msgHtml = '<div class="aui-dialog-body"><input type="text" class="aui_input" placeholder="'+params.text+'"></div>';
            }else{
            ...
            if(dialogButtons && dialogButtons.length > 0){
                for(var ii = 0; ii < dialogButtons.length; ii++){
                    dialogButtons[ii].onclick = function(){
                        if(callback){
                            if(params.input){
                                callback({
                                    ...
                                    text: document.querySelector("input.aui_input").value
                                });
                            }else{
                                callback({
                                    buttonIndex: parseInt(this.getAttribute("button-index"))+1
                        ...
                        if (this.className.indexOf('clicked-keep-open') > -1) {// 自定义事件，点击按钮不关闭窗口
                        } else {
                            self.close();
                        }
                        return;
